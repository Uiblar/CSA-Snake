...
namespace SnakeGame {
    ...
    internal class Snake {
        ...
        internal bool DetectWallCollision(int BoardWidth, int BoardHeight) {
            ...
            int scoreBoxX = BoardWidth - 47;
            int scoreBoxY = 2;
            int scoreBoxWidth = 45;
            int scoreBoxHeight = 15;
            if (head.X >= scoreBoxX && head.X < scoreBoxX + scoreBoxWidth &&
                head.Y >= scoreBoxY && head.Y < scoreBoxY + scoreBoxHeight) {
                Console.WriteLine("Hit the score box!");
                return true; // Collided with the score box
            }
            return false;
        }
